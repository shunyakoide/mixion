  ...
  return (
    <div className="space-y-2 text-sm">
      <div className="flex items-baseline justify-between">
        <span className="font-medium">切り出したコマ</span>
      ...
      {missingPages.length > 0 ? (
        <ul className="space-y-1 text-ink-2">
          {missingPages.map((m) => (
            <li key={m.page}>
              P{m.page}（{frameLabel(m.range[0], settings.frameCount)}〜{frameLabel(m.range[1], settings.frameCount)}）が未取得
            </li>
          ))}
        </ul>
      ) : (
        <p className="text-ok">すべてのページを切り出しました</p>
      )}
    </div>
  )
}
