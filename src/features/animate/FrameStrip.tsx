        ...
        {urls.map((u, i) => (
          <li key={i}>
            <button
              type="button"
              onClick={() => onSelect(i)}
              aria-label={`${frameLabel(i + 1, settings.frameCount)} (${t.animate.source[resolved.sources[i]]})`}
              aria-current={i === current ? 'true' : undefined}
              className={['relative block w-full overflow-hidden rounded-lg bg-ink transition-shadow', i === current ? CURRENT : RING[resolved.sources[i]]].join(' ')}
            >
              <img src={u} alt="" className="aspect-video w-full object-cover" loading="lazy" />
              <span className="absolute left-1.5 top-1 font-mono text-[10px] leading-[14px] text-white [text-shadow:0_0_3px_rgba(0,0,0,.8)]">{frameLabel(i + 1, settings.frameCount)}</span>
            </button>
          </li>
        ))}
      </ol>
