...
export const en = {
  ...
  header: {
    steps: 'Steps',
    sampleMode: 'Trying the sample',
    backToStartShort: 'Start over',
  ...
  steps: {
    ...
    scan: 'Scan',
    animate: 'Animate',
    hintPrintDone: 'PDF saved',
    hintPrintHasVideo: 'Save the print PDF',
    hintPrintNoVideo: 'Load a video to make the PDF',
    hintScanApplied: (applied: number, total: number | null) => `${applied}${total !== null ? ` / ${total}` : ''} frames cut`,
    hintScanImported: 'Click the corners that were not found, then apply',
    hintScanEmpty: 'Import your scanned pages',
    hintAnimateExported: 'Exported',
    hintAnimateReady: 'Export as MP4 or GIF',
    hintAnimateEmpty: 'Cut frames in Scan to export',
    back: (label: string) => `Back: ${label}`,
  ...
  settings: {
    ...
    summary: (frames: number, pages: number) => `${frames} frames → ${pages} pages`,
    layoutLine: (landscape: boolean, w: string, h: string, id: string) => `A4 ${landscape ? 'landscape' : 'portrait'} · frame ${w}×${h} mm · Project ${id}`,
  ...
  scan: {
    ...
    missingPage: (page: number, from: string, to: string) => `P${page} (${from}–${to}) missing`,
  ...
  animate: {
    emptyTitle: 'No frames to animate yet',
    emptyBody: 'Import pages in Scan, set the corners and apply. Then you can play and export here.',
    goScan: 'Go to Scan',
    ...
    original: 'Original',
    originalDrop: 'Original video (optional: audio and fill-in for unscanned frames)',
    framesTitle: 'Frames',
